<?php
  ...
  function evitarDuplicados($email,$conn){
    $sql = "SELECT * FROM registro_docentes where email = '$email'";
    $result = mysqli_query($conn,$sql);
    if(mysqli_num_rows($result)>0){
      return 1;
    }else{
  ...
  if (!empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['confirm_password'])) {
    if(evitarDuplicados($_POST["email"],$conn) == 0){
      if(buscaRepetido($_POST["email"],$conn) == 1){
        if($_POST['password'] == $_POST['confirm_password']){
          $email = $_POST['email'];
          $password = password_hash($_POST['password'], PASSWORD_BCRYPT);
          $query = "INSERT INTO registro_docentes (email,password) VALUES ('$email','$password')";
        ...
        <div class="my-container">
          <div class="form2">
            <form action="registro_docente.php" method="POST">
              ...
              <input name="email" type=text placeholder="Ingrese su correo:" value = "<?php if(isset($_POST['email']) && !$message2) echo $_POST['email'] ?>">
              ...
              <a href="login_docente.php">Inicie sesión </a> aqui</p>
            </form>
          </div>
